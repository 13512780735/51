...
public abstract class PlatformPageAdapter extends ViewPagerAdapter implements OnClickListener {
	...
	private View createPanel(Context context) {
		...
		llPanel.setBackgroundResource(R.drawable.shape_share_bg);
		...
		for (int i = 0; i < lineCount; i++) {
			...
			lp.leftMargin = ScreenUtils.px2dp(context,40);
			lp.rightMargin = ScreenUtils.px2dp(context,40);
			...
			for (int j = 0; j < lineSize; j++) {
				...
				llCells[i * lineSize + j].setBackgroundResource(R.drawable.shape_share_bg);
				lp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, cellHeight);
				lp.weight = 1;
